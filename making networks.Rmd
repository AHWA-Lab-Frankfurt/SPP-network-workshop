...
>>>>>>> 9ad35048f0f72ba2304f81be668d1872f74b85ce
...
threshgraph %>% 
   ...
   mutate(deg = centrality_degree(), # add a column called 'deg' to the node list, which contains degree centrality
          wdeg = centrality_degree() # add a column that contains weighted degree centrality
          )
```
...
When weights are taken into account those edges with a stronger weight are often the ones with the highest betweenness as they are prefered for the pathing.
